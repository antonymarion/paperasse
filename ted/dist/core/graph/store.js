...
function ladybugDbPath(indexDir) {
    ...
    if (!fs.existsSync(dbPath) && fs.existsSync(legacyPath)) {
        ...
        for (const ext of ['.wal', '.shadow', '.tmp']) {
            ...
            if (fs.existsSync(legacySide))
                fs.renameSync(legacySide, newSide);
        }
    }
    return dbPath;
}
...
async function syncLadybug(conn, graph) {
    await conn.query('MATCH (n:GraphNode) DETACH DELETE n');
    const createNodes = await conn.prepare('UNWIND $rows AS row CREATE (n:GraphNode {id: row.id, label: row.label, name: row.name, properties: row.props})');
    ...
    const createEdges = await conn.prepare(`UNWIND $rows AS row
    ...
    async open() {
        ...
        try {
            ...
            if (!fs.existsSync(this.jsonPath)) {
                fs.writeFileSync(this.jsonPath, JSON.stringify({ nodes: [], edges: [] }, null, 2), 'utf-8');
            }
        }
    }
    async saveGraph(graph, meta) {
        ...
        if (this.engine === 'ladybug' && this.conn) {
            try {
                await syncLadybug(this.conn, deduped);
            }
            catch (err) {
                console.warn('[ted] LadybugDB sync ignorée:', err.message);
            }
        }
    }
    loadGraphJson() {
    ...
    async query(text, limit = 12) {
        ...
        const scored = graph.nodes.map((n) => {
            ...
            return {
                ...
                name: n.name,
                score,
                excerpt: n.properties['path'] ?? n.name,
...
export function defaultIndexDir(repoRoot) {
    return path.join(repoRoot, '.ted');
}
export function ladybugDatabasePath(repoRoot) {
