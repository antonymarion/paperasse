...
import { datasetsToGraph, fetchDatagouvDatasets, } from '../connectors/datagouv.js';
export async function runAnalyze(opts = {}) {
    ...
    if (opts.datagouv !== false) {
        try {
            const datasets = await fetchDatagouvDatasets(opts.datagouvQuery ?? 'fiscalité impôt comptabilité');
            datagouvCount = datasets.length;
            graph = mergeGraphs(graph, datasetsToGraph(datasets));
        }
        catch (err) {
            console.warn('[ted] data.gouv.fr ignoré:', err.message);
        }
    }
...
export function resolveRepo(cwd = process.cwd()) {
    ...
    while (dir !== path.dirname(dir)) {
        if (fs.existsSync(path.join(dir, 'comptable', 'SKILL.md')) ||
