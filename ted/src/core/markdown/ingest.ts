...
function extractConcepts(text: string): string[] {
  ...
  for (const m of text.matchAll(/\*\*([^*]{3,80})\*\*/g)) concepts.add(m[1].trim());
  for (const m of text.matchAll(/`([A-Za-z0-9_./-]{3,60})`/g)) concepts.add(m[1].trim());
  for (const m of text.matchAll(/\b(20\d{2}[A-Z0-9-]{0,12})\b/g)) concepts.add(m[1]);
  return [...concepts].slice(0, 40);
}

function extractLinks(text: string, baseDir: string): string[] {
  const links: string[] = [];
...
export async function buildGraphFromRepo(repoRoot: string): Promise<KnowledgeGraph> {
  ...
  for (const skillDir of SKILL_DIRS) {
    ...
    for (const md of refs) {
      if (path.basename(md) === 'SKILL.md') continue;
...
export function mergeGraphs(base: KnowledgeGraph, extra: KnowledgeGraph): KnowledgeGraph {
